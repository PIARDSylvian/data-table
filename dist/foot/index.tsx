...
export default function Foot({
    ...
    for (let idx = 1; idx <= step; idx++) {
        if (idx >= index - 1 && idx < index + 2) {
            paginateButton.push(
                <button
                    className={index === idx ? style['current'] : ''}
                    key={`page-${idx}`}
    ...
    useEffect(
        () =>
            changePaginatedData(
                table.slice((index - 1) * entries, index * entries)
            ),
        [index, entries, table]
    )
    ...
    return (
        <div className={style['table-footer']}>
            <div>
                <p>{`Showing ${
                    table.length === 0 ? 0 : (index - 1) * entries + 1
                } to ${
                    index * entries > table.length
                        ? table.length
                        : index * entries
                } of ${table.length} entries`}</p>
            </div>
            <div data-testid="paginateGroup">
                <button
                    onClick={() => paginate(index - 1)}
                    className={step <= 1 ? style.disabled : ''}
                >
                    Previous
                </button>
                {paginateButton}
