...
const changePaginatedData = (newData:any)=>{paginatedData = newData}
const testArray=[{
  1 : 'bc',
  2 : 'cc'
},{
...
describe('Footer integration test', () => {
  it('should by render with empty data array', () => {
    render(<Foot table={[]} entries={1} changePaginatedData={changePaginatedData}/>)
    ...
    expect(paginateGroup.innerHTML).toEqual('<button>Previous</button><button>Next</button>')
  });

  it('should by renders with data array 10 entries', () => {
    render(<Foot table={testArray} entries={10} changePaginatedData={changePaginatedData}/>)
    const footerText = screen.getByText(/Showing 1 to 3 of 3 entries/i)
    expect(paginatedData).toEqual(testArray)
    expect(footerText).toBeInTheDocument()

    const paginateGroup = screen.getByTestId('paginateGroup')
    expect(paginateGroup.innerHTML).toEqual('<button>Previous</button><button class="current">1</button><button>Next</button>')
  });

  it('should by renders with data array 1 entries', () => {
    render(<Foot table={testArray} entries={1} changePaginatedData={changePaginatedData}/>)
    let footerText = screen.getByText(/Showing 1 to 1 of 3 entries/i)
    expect(paginatedData).toEqual([testArray[0]])
    expect(footerText).toBeInTheDocument()
    const paginateGroup = screen.getByTestId('paginateGroup')
    expect(paginateGroup.innerHTML).toEqual('<button>Previous</button><button class="current">1</button><button class="">2</button><button>Next</button>')
